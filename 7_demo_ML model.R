############################################################################################################# 7.
##### 7. ML modelling (Fig.2; Table S9-11; Fig.S10&12)
setwd("~/Desktop") 
rm(list=ls()) #clear all variables in workplace
library(dplyr)
library(tidyverse)
library(foreach) # 7.1 - ML models
library(doParallel) # 7.1 - ML models
library(doRNG) # 7.1 - ML models
library(patchwork) # 7.3.1 - Figures
library(ggpattern) # 7.3.1 - Figures
library(ggpointdensity) # 7.3.2 - Figures (Density&calibration)
library(extrafont) # 7.3.2 - Figures (Font)
library(ggcorrplot) # 7.3.4 - Correlation
...
## Define all models - Fig.2; Table S9; Fig.S10
model_list <- list(
...
############################################################################################################# 7.2.1
######### 7.2.1 Data extractions - Evaluation metrics 95% CI - Table S9 ########
...
# 
# all_avg[31, 3] <- 0.03201584 # GLMNET model 5, test
# all_avg[34, 3] <- 0.07272433 # GLMNET model 5, train
# 
...
view(df_final) # Table S9

############################################################################################################# 7.2.2
################### 7.2.2 Data extractions - hyperparameters ###################
...
ggplot(all_hyper_metric, aes(x = lambda, y = alpha, color = R2)) +
  ...
  theme(
    ...
    strip.text = element_text(size = 6)) # Fig.S10a

############################################################################################################# 7.2.3
############### 7.2.3 Data extractions - importance/coefficients ###############
...
############################################################################################################# 7.2.4
#### 7.2.4 Data extractions - p values b/t models on test sets - Table S10 #####
df_test <- all_metrics %>%
  filter(Set == "Test") %>% 
...
long_df_T1_3 <- df_test %>% 
  pivot_longer(cols = c("model0","model1","model2","model3","model4","model5",
...
p_value <- pairwise.wilcox.test(long_df_T1_3$R2,  # Change R2/RMSE/MAE
                                long_df_T1_3$Model, paired = TRUE, p.adjust.method = "none")  # Table S10
view(p_value$p.value)

############################################################################################################# 7.3.1
######################## 7.3.1 Bar plots Fig. 2 - GLM ##########################
labels_fig.2 <- c(
  model0 = "Baseline*",
  model1 = "WC",
  model2 = "Clinical (with WC)",
...
############################################################################################################# 7.3.2
################# 7.3.2 Calibration plots - GLM - Fig.S10b&c ###################
...
metrics <- df_final%>% # df_final is in 7.3.1 - Step 6; for Fig.S10b
...
ggplot(calibration_data_fig.S10, aes(x = Predicted, y = Actual)) +
  ...
  theme(
    ...
    legend.text = element_text()) # Fig.S10b

## Residual plots for all models  # Fig.S10c
...
ggplot(all_residual_data, aes(x = Predicted, y = Residual)) +
  ...
  theme(
    ...
    strip.text = element_text(family = "Times New Roman", size = 8)) # Fig.S10c

############################################################################################################# 7.3.3
##### 7.3.3 Figures for the best model - importance - Fig.S11a; Table S11 #######
...
invisible(lapply(unique(all_importance$Model), function(model) {
  print(plot_var_importance(model))})) # Fig.S11a

## Find standardized beta coefficients
find_var_coef <- function(variable_names, model_name = NULL) {
  if (is.null(model_name)) {
    # Check across all models for multiple variables
    result <- all_importance %>%
      ...
      arrange(Variable, desc(abs(Mean_Coefficient)))
  } else {
    # Check in specific model for multiple variables
    result <- all_importance %>%
      filter(Model == model_name, Variable %in% variable_names) %>%
      group_by(Variable) %>%
      summarize(
        Mean_Coefficient = mean(Importance),
        SD = sd(Importance),
...
find_var_coef(c("InsulinpmolL", "SBP"), "model3") # Table S11
find_var_coef(c("Tyr_metabolite", "Phe_metabolite","L_HDL_CE_metabolite",
                "S_LDL_P_metabolite","GlycA_metabolite","Albumin_metabolite"), "model4")  # Table S11

############################################################################################################# 7.3.4
################# 7.3.4 Correlation plot - Fig.S11b; Table S11 ##################
cor_matrix <- df %>% 
...
variables <- c("InsulinpmolL", "SBP", "Albumin_metabolite", "Tyr_metabolite", 
